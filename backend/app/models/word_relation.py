...
class WordRelation(Base):
    ...
    meaning_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("meanings.id", ondelete="CASCADE"), nullable=True, index=True
    )
    ...
    related_word_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("words.id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    related_word_entry: Mapped["Word | None"] = relationship(
        "Word",
        foreign_keys=[related_word_id],
    )
