...
describe("apiClient auth lifecycle", () => {
  ...
  it("refreshes once on 401 and retries the original request", async () => {
    ...
    expect(window.localStorage.getItem("words_admin_refresh_token")).toBe("next-refresh");
  });
  ...
  it("clears token and redirects to login when refresh fails", async () => {
    ...
    expect(window.localStorage.getItem("words_admin_access_token")).toBeNull();
    expect(window.localStorage.getItem("words_admin_refresh_token")).toBeNull();
