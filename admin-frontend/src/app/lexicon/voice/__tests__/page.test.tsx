...
import LexiconVoicePage from "@/app/lexicon/voice/page";
import { useRouter } from "next/navigation";
import { redirectToLogin } from "@/lib/auth-redirect";
import { readAccessToken } from "@/lib/auth-session";
import { getLexiconVoiceRunDetail, getLexiconVoiceRuns, getLexiconVoiceStoragePolicies, rewriteLexiconVoiceStorage } from "@/lib/lexicon-ops-client";
...
describe("LexiconVoicePage", () => {
  ...
  beforeEach(() => {
    ...
    window.history.pushState({}, "", "/lexicon/voice");
  });

  it("redirects unauthenticated users to login", async () => {
    mockReadAccessToken.mockReturnValue(null);
    render(<LexiconVoicePage />);
    await waitFor(() => expect(mockRedirectToLogin).toHaveBeenCalledWith("/lexicon/voice"));
  });
  ...
  it("shows runs separately and supports policy-specific dry-run rewrite", async () => {
    const user = userEvent.setup();
    render(<LexiconVoicePage />);
