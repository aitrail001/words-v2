...
export default function LexiconPage() {
  ...
  useEffect(() => {
    ...
    if (!hasToken) {
      redirectToLogin("/lexicon/review");
    }
  }, []);
  ...
  return (
    <div className="space-y-6" data-testid="lexicon-admin-page">
      <div>
        <h2 className="text-2xl font-bold" data-testid="lexicon-admin-title">Legacy Staged Review</h2>
        <p className="text-sm text-gray-600">Use this legacy tool only when you explicitly generate `selection_decisions.jsonl` for staged review. The current primary operator surfaces are Lexicon Words and Lexicon Operations.</p>
      </div>
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900" data-testid="lexicon-admin-workflow-note">
        <p className="font-medium">Legacy workflow note</p>
        <p>Staged review batches are separate from the main word tables. Keep using this page only for optional legacy review flows; current real 30K snapshot runs may not produce `selection_decisions.jsonl` at all.</p>
      </div>
