...
test("admin can review and export a compiled lexicon batch", async ({ page, request }) => {
  const user = await registerAdminViaApi(request, "admin-compiled-review-full");
  ...
  await page.getByTestId("compiled-review-decision-reason").fill("approved in compiled review full");
  ...
  expect(approvedItem?.decision_reason).toBe("approved in compiled review full");
  ...
  expect(decisionLines).toEqual([
    expect.objectContaining({
      ...
      decision: "approved",
      decision_reason: "approved in compiled review full",
    }),
  ]);
