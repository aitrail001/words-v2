...
test("admin can bulk approve a compiled lexicon batch with progress", async ({ page, request }) => {
  ...
  await page.getByTestId("compiled-review-decision-reason").fill("bulk approved in full");
  ...
  expect(itemsPage.items.every((item) => item.decision_reason === "bulk approved in full")).toBeTruthy();
